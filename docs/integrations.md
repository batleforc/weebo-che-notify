...
node /projects/weeboBridgeNotify/bin/ide-notify info "Message"
echo '{"message": "Message"}' | node /projects/weeboBridgeNotify/bin/ide-notify
```
...
```toml
notify = ["node", "/projects/weeboBridgeNotify/bin/ide-notify"]
```
...
2. Le pointer sur `node /projects/weeboBridgeNotify/bin/ide-notify` :
...
if [ $code -eq 0 ]; then
  node /projects/weeboBridgeNotify/bin/ide-notify info "MonAgent : terminé ✅"
else
  node /projects/weeboBridgeNotify/bin/ide-notify error "MonAgent : échec (exit $code)"
fi
```
...
tasks:
  build:notify:
    cmds:
      - defer: node /projects/weeboBridgeNotify/bin/ide-notify info "Build terminé"
      - task: build
```
...
```bash
node /projects/weeboBridgeNotify/bin/ide-notify info "Build terminé ✅" \
